...
var Title = function() {
	...
	this.interval = setInterval(function() {
		{ // Render
			// Background
			background(title.world.backdrop.r, title.world.backdrop.g, title.world.backdrop.b);

			// Shadows
			fill(title.world.backdrop.r - 20, title.world.backdrop.g - 20, title.world.backdrop.b - 20);
			noStroke();
			rect(title.world.x + title.world.width / 2 + 7, title.world.y + title.world.height / 2 + 6, title.world.width, title.world.height); // World

			// World
			fill(title.world.background.r, title.world.background.g, title.world.background.b);
			stroke(title.world.border.color.r, title.world.border.color.g, title.world.border.color.b);
			strokeWeight(1);
			rect(title.world.x + title.world.width / 2, title.world.y + title.world.height / 2, title.world.width, title.world.height);

			// Orgs
			renderOrgs();
		}
		{ // Calculate
			for (let i = 0; i < title.orgs.length; i++) {
				grow(title.orgs[i]);
			}
		}
	}, _ofrequency);
