...
var Cell = function(X, Y, orG) {
	...
	this.r = function(orG) { // Distance from org center
		let distance = sqrt(sq(this.x - game.orgs[orG.index].x()) + sq(this.y - game.orgs[orG.index].y()));
		return distance;
	};
	this.d = function(orG) { // Distance from target (Position in world)
		let distance = sqrt(sq(this.x - game.orgs[orG.index].pos.x) + sq(this.y - game.orgs[orG.index].pos.y));
		return distance;
	};
};